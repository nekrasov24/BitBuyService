using AutoMapper;
using BitBuy.HeaderServise;
using BitBuy.Models;
using BitBuy.Repository;
using BitBuy.SmsService;
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<string> Register(RegisterRequest model)
        {
            try
            {
                ...
                var newUser = new Models.User
                {
                    ...
                    TwoFactorAuthenticationSwitch = TwoFactorAuthenticationSwitch.TurnedOff,
                };
        ...
        public async Task<string> Authenticate(AuthenticateRequest model)
        {
            try
            {
                ...
                if (user.TwoFactorAuthenticationSwitch == TwoFactorAuthenticationSwitch.TurnedOn)
                {
                    var code = GenerateSmsCode();
                    _smsService.SendSms(code, user.Phone);
                    user.TwoFactorCode = code;
                    var userCode = user.TwoFactorCode;
        ...
        public async Task<string> VerifyTwoFactorCode(TwoFactorRequestModel model)
        {
            var user = (await _userRepository.GetAllAsync(u => u.Email.ToUpper().Equals(model.Email.ToUpper()))).FirstOrDefault();
        ...
        private string GenerateSmsCode()
        {
            ...
            var result = random.Next(100000, 999999).ToString();
            return result;
        }
